...
Quill now ships as a Capacitor Android app in addition to the browser/PWA build.

## Download from GitHub

Every push to `main` runs the **Android APK** workflow. Open the latest successful run in GitHub Actions and download the `quill-debug-apk` artifact. The artifact contains `app-debug.apk`, which can be installed on Android after enabling installs from unknown sources for the browser or file manager you use.
...
- The app ID is `com.thecameronboyer.quill`.
